...
const KategoriList = () => {
    ...
    const [kategoriFull, setKategoriFull] = useState([]);
    ...
    const handleSearch = (e) => {
        ...
        if (keyword === '') {
            setKategori(kategoriFull);
        } else {
            const filtered = kategoriFull.filter((item) =>
    ...
    const openEdit = (item) => {
        setFormData({ id: item.id, nama_kategori: item.nama_kategori });
        setIsEditing(true);
        setShowModal(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await kategoriAPI.update(formData.id, { nama_kategori: formData.nama_kategori });
                setSuccess('✅ Kategori berhasil diupdate!');
            } else {
                await kategoriAPI.create({ nama_kategori: formData.nama_kategori });
        ...
        <div style={styles.container}>
            <div style={styles.header}>
                <div style={styles.headerLeft}>
                    ...
                    <h1 style={styles.title}>📂 Data Kategori</h1>
                </div>
                <button style={styles.addBtn} onClick={openAdd}>+ Tambah Kategori</button>
            </div>
            ...
            <div style={styles.searchContainer}>
                ...
                />
                <span style={styles.totalData}>Total: {kategori.length} kategori</span>
            </div>
            ...
            <div style={styles.tableWrapper}>
                <table style={styles.table}>
                    ...
                    <tbody>
                        {kategori.length === 0 ? (
                            <tr>
                                <td colSpan="3" style={styles.emptyData}>📭 Belum ada data</td>
                            </tr>
                        ) : (
                            kategori.map((item, index) => (
                                <tr key={item.id} style={styles.tr}>
                                    <td style={styles.td}>{index + 1}</td>   {/* ← NOMOR URUT */}
                                    <td style={styles.td}><strong>{item.nama_kategori}</strong></td>
                                    <td style={styles.td}>
                                        <button style={styles.editBtn} onClick={() => openEdit(item)}>✏️ Edit</button>
                                        <button style={styles.deleteBtn} onClick={() => handleDelete(item.id, item.nama_kategori)}>🗑️ Hapus</button>
                                    </td>
                                </tr>
                            ))
                        )}
                ...
                <div style={styles.modalOverlay} onClick={() => setShowModal(false)}>
                    <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                        <div style={styles.modalHeader}>
                            <h2>{isEditing ? '✏️ Edit Kategori' : '➕ Tambah Kategori'}</h2>
